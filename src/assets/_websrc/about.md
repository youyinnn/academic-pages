## [Jun Huang](https://scholar.google.ca/citations?user=jvS_ViUAAAAJ&hl=en)

### 1. Research Focus

- **Explainable AI: Methods and Trustworthy Evaluation** for Large-Scale Machine Learning and Operations (MLOps) Workflow
- **AI Governance of Machine Learning and Explainability Operations (MLXOps): Data Provenance and Sustainable Operations**

### 2. Education

> 2023 (Summer) - Now, ECE, Ph.D. Candidate: '', '', ''.
>
> 2022 (Winter) - 2023 (Winter), ECE, MASc.: [Concordia University](https://www.concordia.ca/), supervised by Dr. [Yan Liu](https://www.concordia.ca/faculty/yan-liu.html).
>
> 2021 (Fall), ECE, MEng: [Concordia University](https://www.concordia.ca/), Montreal, Canada.
>
> 2015-2019, SE, BE : [South-Central Minzu University (SCM)](https://www.scuec.edu.cn/), Wuhan, China.

### 3. Publications

#### 3.1 Journal

- **Huang, J**., & Liu, Y. (**2025**). MLXOps4Medic: A Service Framework for Machine Learning and Explaining Operations in Medical Imaging AI Development. _IEEE Access_.
- Li, D., Liu, Y., & **Huang, J**. (**2024**). Assessment of Software Vulnerability Contributing Factors by Model-Agnostic Explainable AI. _Machine Learning and Knowledge Extraction_, _6_(2), 1087-1113.
- Wang, Z., Liu, Y., & **Huang, J**. (**2024**). An Open API Architecture to Discover the Trustworthy Explanation of Cloud AI Services. IEEE Transactions on Cloud Computing, 12, 762-776.
- Li, D., Liu, Y., **Huang, J**., & Wang, Z. (**2023**). A Trustworthy View on Explainable Artificial Intelligence Method Evaluation. Computer, 56, 50-60.

#### 3.2 Conference

- Liu, Y., **Huang, J**., Hamou-Lhadj, A., & Li, Z. (**2026**). Redundancy as the Shadow of Explainability: A Trade-Off Principle for AI-Intensive Systems. The 48th International Conference of Software Engineering. ICSE 2026.
- **Huang, J**., & Liu, Y. (**2026**). PROV-MLXOps: Towards Provenance-Based Machine Learning and Explainability Operations. Service-Oriented Computing. ICSOC 2025. The 23rd International Conference on Service-Oriented Computing, Shenzhen, China.
- **Huang, J**., & Liu, Y. (**2024**). Guided AbsoluteGrad : Magnitude of Gradients Matters to Explanation’s Localization and Saliency. Proceedings of the Canadian Conference on Artificial Intelligence. https://caiac.pubpub.org/pub/8rzaolsn
- Neghawi, E., Wang, Z., **Huang, J**., & Liu, Y. (**2023**). Linking Team-level and Organization-level Governance in Machine Learning Operations through Explainable AI and Responsible AI Connector. 2023 IEEE 47th Annual Computers, Software, and Applications Conference (COMPSAC), 1223-1230.
- **Huang, J**._, Wang, Z._, Li, D., & Liu, Y. (**2022**). The Analysis and Development of an XAI Process on Feature Contribution Explanation. 2022 IEEE International Conference on Big Data (Big Data), 5039-5048.

### 4. Public Talks

- 2025 APSEC Tutorial: [Architecting Large-Scale Explainable AI (XAI) Software: Process, Pipeline, and Framework](https://conf.researchr.org/track/apsec-2025/apsec-2025-tutorials?#)
- 2024 CASCON The 1st International Workshop on Observability and Explainability of AI Systems (OXAI'24): How do we efficiently integrate explainable AI (XAI) to help the deep learning of medical images?
- 2024 Canadian AI Main Conference: Guided AbsoluteGrad: Magnitude of Gradients Matters to Explanation's Localization and Saliency
- 2023 Canadian AI Responsible AI Track: The Analysis and Development of an XAI Process on Feature Contribution Explanation.
- [2022 IEEE International Conference on Big Data (Big Data): The Analysis and Development of an XAI Process on Feature Contribution Explanation](https://www.youtube.com/watch?v=j8I9Neksq9E)

### 5. Academic Services

#### 5.1 Concordia

Teaching Assistant:

- 2026:
  - Winter, COEN244, PROGRAMMING METHODOLOGY II
- 2025:
  - Fall, COEN424, PROGRAMMING ON THE CLOUD
  - Winter, COEN448, SOFTWARE TESTING AND VALIDATION
- 2024:
  - Fall, COEN6311, SOFTWARE ENGINEERING
  - Summer, COEN424, PROGRAMMING ON THE CLOUD
- 2023:
  - Fall, COEN448, SOFTWARE TESTING AND VALIDATION
  - Fall, COEN424, PROGRAMMING ON THE CLOUD
  - Summer, COEN448, SOFTWARE TESTING AND VALIDATION
  - Winter, COEN6731, DISTRIBUTED SOFTWARE SYSTEMS
  - Winter, COEN448, SOFTWARE TESTING AND VALIDATION
- 2022:
  - Fall, COEN352, DATA STRUCTURES AND ALGORITHMS
  - Summer, COEN244, PROGRAMMING METHODOLOGY II

#### 5.2 SCMU Software Engineering Lab in Computer Science College

Member and Technical Leader of Java EE Group:

- Sep. 2016 – Feb. 2019:
  - Led the technical and made progress together with team members;
  - Hold the recruitment and assessment of new staff;
  - Seek cooperation with other teams on competitions and projects;
  - Participate in software and computer science competitions;
